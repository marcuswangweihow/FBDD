...
run_id = "4MZI_100ps_checkpoint_testing_A"
...
for f in hills_files:
    path = os.path.join(gmx_temp_dir, f)
    if os.path.exists(path):
        print(f"Truncating HILLS file {path}")
...
# truncate_edr(os.path.join(gmx_temp_dir, "prod.edr"), last_time_ps, gmx_bin=gmx_bin)
...
subprocess.run([gmx_bin, "mdrun", "-deffnm", "prod", "-v", "-plumed", plumed_file, "-cpt", checkpoint_interval, "-append"] + cpi_arg,
